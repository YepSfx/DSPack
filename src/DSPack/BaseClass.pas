...
// milenko end
    ...
    function NotifyEvent(EventCode : Integer ; EventParam1, EventParam2: NativeInt): HRESULT;
...
function TBCBaseFilter.NotifyEvent(EventCode : Integer ; EventParam1,
  EventParam2: NativeInt): HRESULT;
var
  Filter : IBaseFilter;
begin
  ...
  begin
    ...
    if (EC_COMPLETE = EventCode) then EventParam2 := NativeInt(Filter);
...
begin
  ...
  {$ELSE}
    ...
    {$ENDIF}
    {$IFDEF WIN64}
    Thread := InterlockedExchange64(Int64(FThread), 0);
    {$ENDIF}
  {$ENDIF}
  // martin end
  if BOOL(Thread) then
...
// milenko start (Delphi 5 compatibility)
      QueryInterface(IID_IBaseFilter,Filter);
      NotifyEvent(EC_COMPLETE, S_OK, NativeInt(Filter));
      Filter := nil;
// milenko end
      FState := State_Running;
      Result := NOERROR;
...
// milenko start (Delphi 5 compatibility)
    QueryInterface(IID_IBaseFilter,Filter);
    Result := NotifyEvent(EC_COMPLETE, S_OK, NativeInt(Filter));
    Filter := nil;
// milenko end
  finally
    FRendererLock.UnLock;
...
//      Pin := FInputPin as IPin;
      ...
      NotifyEvent(EC_REPAINT, NativeInt(Pin), 0);
      Pin := nil;
// milenko end
      SetRepaintStatus(False);
{$IFDEF DEBUG}
...
//  Pin := FInputPin as IPin;
    FInputPin.QueryInterface(IID_IPin,Pin);
    // ??? m_pInputPin->AddRef();
    NotifyEvent(EC_DISPLAY_CHANGED, NativeInt(Pin), 0);
    SetAbortSignal(True);
    ClearPendingSample;
//    FreeAndNil(FInputPin);
    Pin := nil;
...
begin
  ...
  begin
    ...
    QueryInterface(IID_IBaseFilter, Filter);
    NotifyEvent(EC_WINDOW_DESTROYED, NativeInt(Filter), 0);
    Filter := nil;
  end;
